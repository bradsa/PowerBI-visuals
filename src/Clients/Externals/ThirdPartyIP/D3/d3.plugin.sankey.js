d3.sankey = function () {
    ...
    sankey.scaleFactor = function (_) {
        if (!arguments.length) return scaleFactor;
        scaleFactor = _;
        return sankey;
    };

    sankey.cycleEnabled = function (_) {
        if (!arguments.length) return cycleEnabled;
        cycleEnabled = _;
        return sankey;
    };
    ...
    function computeNodeDepths(iterations) {
        ...
        function initializeNodeDepth() {
            ...
            links.forEach(function (link) {
                link.dy = (link.value * ky) * scaleFactor;
            });
        }
    ...
    function createsCycle(originalSource, nodeToCheck, graph) {
        if (originalSource === nodeToCheck) { // self- cycle
            return true;
        }
